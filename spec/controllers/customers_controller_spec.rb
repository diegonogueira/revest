...
describe CustomersController do
  ...
  def mock_customer(stubs={})
    @mock_customer ||= mock_model(Customer, :representative_id => "12")
  end

  describe "GET index" do
    it "assigns all customers as @customers" do
      Customer.stub!(:paginate).and_return([mock_customer])
      get :index, :representative_id => "12"
  ...
  describe "DELETE destroy" do
    it "destroys the requested customer" do
      Customer.should_receive(:find).with(:first,:conditons=>{:id=>"37"}).and_return(mock_customer)
